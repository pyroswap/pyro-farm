...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await PyroReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.pyro.address, '100', '1000', { from: owner });

        await this.pyro.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setPyroReferral(this.referral.address, { from: owner });
